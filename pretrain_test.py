...
test_dataset = dtld.Dataset_('/home/user/Рабочий стол/stereo_imgs/', test_files_name, disp_map=False)  
test_dataloader = DataLoader(test_dataset, batch_size=bsize, num_workers=1)



device = 'cuda'
model = torch.load('models/best_model.pt', map_location='cpu').to(device).eval()
